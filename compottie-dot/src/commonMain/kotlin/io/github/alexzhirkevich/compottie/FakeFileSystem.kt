...
internal class FakeFileSystem() : FileSystem() {
    ...
    private val roots = mutableMapOf<Path, Element.Directory>()
    ...
    private fun list(dir: Path, throwOnFailure: Boolean): List<Path>? {
        ...
        val element = lookupResult.element as? Element.Directory
            ?: if (throwOnFailure) throw IOException("not a directory: $dir") else return null

        element.access(now = currentTime())
    ...
    private fun open(
        ...
        if (readWrite) {
            ...
            element = Element.File(createdAt = existing?.createdAt ?: now)
            parent.children[lookupResult.segment!!] = element
            operation = Operation.WRITE

            if (existing is Element.File) {
                element.data = existing.data
            }
        } else {
            val existing = lookupResult?.element
                ?: throw FileNotFoundException("no such file: $file")
            element = existing as? Element.File ?: throw IOException("not a file: $file")
        ...
        element.access(now = currentTime(), modified = readWrite)
    ...
    override fun createDirectory(dir: Path, mustCreate: Boolean) {
        ...
        parentDirectory.children[canonicalPath.nameBytes] =
            Element.Directory(createdAt = currentTime())
    }
    ...
    override fun atomicMove(
        ...
        if (targetLookupResult?.element is Element.Directory) {
        ...
        if (!allowClobberingEmptyDirectories) {
            if (
                sourceLookupResult?.element is Element.Directory
                && targetLookupResult?.element is Element.File
            ) {
    ...
    override fun delete(path: Path, mustExist: Boolean) {
        ...
        if (lookupResult.element is Element.Directory && lookupResult.element.children.isNotEmpty()) {
    ...
    override fun createSymlink(
        ...
        parent.children[canonicalSource.nameBytes] =
            Element.Symlink(createdAt = currentTime(), target)
    }
    ...
    private fun lookupPath(
        ...
        if (root == null) {
            ...
            root = Element.Directory(createdAt = currentTime())
            roots[rootPath] = root
        }

        var parent: Element.Directory? = null
        ...
        for (segment in segments) {
            ...
            if (current !is Element.Directory) {
            ...
            if (current is Element.Symlink && followSymlinks) {
                current.access(now = currentTime())
    ...
    private fun PathLookupResult?.requireParent(): Element.Directory {
    ...
    private sealed class Element(
        ...
        class File(createdAt: Long) : Element(createdAt) {
            ...
            override val metadata: FileMetadata
                get() = FileMetadata(
                    ...
                    lastModifiedAtMillis = lastModifiedAt,
                    lastAccessedAtMillis = lastAccessedAt,
                    extras = extras,
                )
        }

        class Directory(createdAt: Long) : Element(createdAt) {
    ...
    private inner class FakeFileHandle(
        ...
        override fun protectedResize(size: Long) {
            ...
            file.access(now = currentTime(), modified = true)
        }
        ...
        override fun protectedClose() {
            if (closed) return
            closed = true
            file.access(now = currentTime(), modified = readWrite)
            openFiles -= openFile
        }
