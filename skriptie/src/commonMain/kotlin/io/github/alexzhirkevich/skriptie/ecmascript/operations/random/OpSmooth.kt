...
internal fun OpSmooth(
    ...
    val prop = prop(context) as RawProperty<Any>
    val width = (width?.invoke(context) as Number?)?.toFloat() ?: .4f
    val samples = (samples?.invoke(context) as Number?)?.toInt() ?: 5
    val time = (time?.invoke(context) as Number?)?.toFloat()
